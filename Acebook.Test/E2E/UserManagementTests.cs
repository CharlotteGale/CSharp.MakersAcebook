...
public class UserManagement : PlaywrightTestBase
{
  ...
  [Test]
  public async Task SigningUpUnder13ShowsAgeErrorMessage()
  {
    await Page.GotoAsync("http://127.0.0.1:5287/");
    await Page.GetByRole(AriaRole.Button, new() { Name = "Create new account" }).ClickAsync();

    await Page.GetByPlaceholder("Name").FillAsync("User1");
    await Page.GetByPlaceholder("Email").FillAsync("user1@test.com");
    await Page.GetByPlaceholder("Password").FillAsync("Password1!");

    var twelveYearsAgo = DateTime.UtcNow.AddYears(-12);
    await Page.Locator("input[type='date']").FillAsync(twelveYearsAgo.ToString("yyyy-MM-dd"));

    await Page.Locator("#signup-submit").ClickAsync();

    var error = Page.Locator("span[data-valmsg-for='DateOfBirth']");
    await Expect(error).ToContainTextAsync("You must be at least 13 years old to sign up.");
  }

  [Test]
  public async Task SigningUpExactly13AllowsRegistration()
  {
    await Page.GotoAsync("http://127.0.0.1:5287/");
    await Page.GetByRole(AriaRole.Button, new() { Name = "Create new account" }).ClickAsync();

    var uniqueEmail = $"user2_{Guid.NewGuid():N}@test.com";
    await Page.GetByPlaceholder("Name").FillAsync("User2");
    await Page.GetByPlaceholder("Email").FillAsync(uniqueEmail);
    await Page.GetByPlaceholder("Password").FillAsync("Password1!");

    var thirteenYearsAgo = DateTime.UtcNow.AddYears(-13);
    await Page.Locator("input[type='date']").FillAsync(thirteenYearsAgo.ToString("yyyy-MM-dd"));
  ...
  [Test]
  public async Task SigningUpOver13AllowsRegistration()
  {
    await Page.GotoAsync("http://127.0.0.1:5287/");
    await Page.GetByRole(AriaRole.Button, new() { Name = "Create new account" }).ClickAsync();

    var uniqueEmail = $"user3_{Guid.NewGuid():N}@test.com";
    await Page.GetByPlaceholder("Name").FillAsync("User3");
    await Page.GetByPlaceholder("Email").FillAsync(uniqueEmail);
    await Page.GetByPlaceholder("Password").FillAsync("Password1!");

    var twentyFiveYearsAgo = DateTime.UtcNow.AddYears(-25);
    await Page.Locator("input[type='date']").FillAsync(twentyFiveYearsAgo.ToString("yyyy-MM-dd"));
  ...
  [Test]
  public async Task SigningUpWithMissingDateOfBirthShowsErrorMessage()
  {
    await Page.GotoAsync("http://127.0.0.1:5287/");
    await Page.GetByRole(AriaRole.Button, new() { Name = "Create new account" }).ClickAsync();

    await Page.GetByPlaceholder("Name").FillAsync("User4");
    await Page.GetByPlaceholder("Email").FillAsync("user4@test.com");
    await Page.GetByPlaceholder("Password").FillAsync("Password1!");

    await Page.Locator("#signup-submit").ClickAsync();

    var error = Page.Locator("span[data-valmsg-for='DateOfBirth']");
    await Expect(error).ToContainTextAsync("The Date of Birth field is required.");
  }
}